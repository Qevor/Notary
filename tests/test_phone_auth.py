...
def test_login_page_renders():
    ...
    assert "Email Code" in response.text
    assert "Sandbox Phone" in response.text

@patch("apps.api.main.get_app_service")
def test_send_phone_code(mock_get_service):
    mock_service = AsyncMock()
    mock_service.send_phone_otp.return_value = {"status": "otp_sent", "phone": "+1234567890"}
...
@patch("apps.api.main.get_app_service")
def test_verify_phone_code(mock_get_service):
    mock_service = AsyncMock()
    mock_service.verify_phone_otp.return_value = {
